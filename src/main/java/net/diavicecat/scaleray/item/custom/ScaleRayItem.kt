...
class ScaleRayItem(properties: Properties) : Item(properties) {
    ...
    override fun appendHoverText(stack: ItemStack, context: TooltipContext, lines: MutableList<Component>, flag: TooltipFlag) {
        ...
        if (cell.isEmpty) {
            ...
            val charges = cell.get(net.diavicecat.scaleray.component.ModDataComponents.POWER_CHARGES.get()) ?: PowerCellItem.MAX_CHARGES
            val color = if (charges > 0) 0x55FF55 else 0xFF5555
            lines += Component.literal("Cell: $charges/${PowerCellItem.MAX_CHARGES} charges").withStyle { it.withColor(color) }
        }
    }
    ...
    override fun isBarVisible(stack: ItemStack): Boolean {
        ...
        val charges = cell.get(net.diavicecat.scaleray.component.ModDataComponents.POWER_CHARGES.get()) ?: PowerCellItem.MAX_CHARGES
        return charges < PowerCellItem.MAX_CHARGES
    }
    ...
    override fun getBarWidth(stack: ItemStack): Int {
        ...
        val charges = cell.get(net.diavicecat.scaleray.component.ModDataComponents.POWER_CHARGES.get()) ?: PowerCellItem.MAX_CHARGES
        return (charges.toDouble() / PowerCellItem.MAX_CHARGES * 13).toInt()
    }
